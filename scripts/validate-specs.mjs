...
const machine=read('machine.json'),video=read('video.json'),input=read('input.json'),audio=read('audio.json'),storage=read('storage.json'),abi=read('kernel-abi.json'),assets=read('assets.json'),issues=read('known-issues.json');read('mia-registers.json');read('mia-commands.json');read('source-manifest.json');read('emulator-automation.json');
for(const n of ['machine.schema.json','assets.schema.json','project.schema.json','palette.schema.json','palette-config.schema.json','tileset.schema.json','shape.schema.json','animation.schema.json','load-plan.schema.json'])read(`schema/${n}`);
